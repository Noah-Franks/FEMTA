...
extern int  yylex  ();
extern void yyerror(char * message, ...) opt(noreturn);

typedef struct EffectNode EffectNode;
typedef struct Specification Specification;

EffectNode    * make_pin_effect  (Numeric * wire, bool hot              );
EffectNode    * make_state_effect(char * name, bool enter               );
...
Specification * extend_trigger 
  (List * state_names, char * id, bool less, Numeric * threshold, List * options, Trigger * trigger);

void check_and_add_state         (char * name, bool enter);
...
%code requires {
  ...
  typedef struct EffectNode {          // something that can happen
    
    bool  is_pin_change;               // whether effect changes a pin or state
    float delay_ms;                    // effect delay in ms
    
    union {
      struct {
        PinChange * pin_change;        // the pin we're working with
        bool        hot;               // whether to go hot or cold
      };
      
      struct {
        StateChange * state_change;    // the state we're working with
        bool          enter;           // whether to enter or leave
      };
    };
    
  } EffectNode;
  
  typedef struct Specification {       // describes the behavior of a sensor
    
    char * id;                         // specifications take the form as below:
    List * options;                    //     
    List * args;                       //     [ id, options, args ]
...
Effect   : SET PIN NUMERIC POS                             { $$ = make_pin_effect($3,  true);                     }
         | SET PIN NUMERIC NEG                             { $$ = make_pin_effect($3, false);                     }
         | ENTER ID                                        { $$ = make_state_effect($2,  true);                   }
         | LEAVE ID                                        { $$ = make_state_effect($2, false);                   }
         ;
...
EffectNode * make_pin_effect(Numeric * wire, bool hot) {        // 
  ...
  PinChange  * change = calloc(1, sizeof(*change));
  EffectNode * effect = calloc(1, sizeof(*effect));
  
  change -> gpio = wire -> integer;                             // delays are added later
  
  effect -> pin_change    = change;
  effect -> hot           = hot;
  effect -> is_pin_change = true;
...
EffectNode * make_state_effect(char * name, bool enter) {      //
  ...
  if (!state_get(name))
    yyerror("Unknown state name: " CYAN "%s", name);
  
  StateChange * change = calloc(1, sizeof(*change));
  EffectNode  * effect = calloc(1, sizeof(*effect));
  
  change -> state = state_get(name);                            // delays are added later
  
  effect -> state_change  = change;
  effect -> enter         = enter;
  effect -> is_pin_change = false;
...
EffectNode * add_delay(EffectNode * effect, Numeric * delay) {
  ...
  if (effect -> is_pin_change) effect ->   pin_change -> delay = (int) effect -> delay_ms;
  else                         effect -> state_change -> delay = (int) effect -> delay_ms;
...
Trigger * make_trigger(List * effects) {
  ...
  trigger -> enter_set  = list_that_frees(free);
  trigger -> leave_set  = list_that_frees(free);
  
  for (iterate(effects, EffectNode *, effect))
    if (effect -> is_pin_change)
      if (effect -> hot  ) list_insert(trigger -> wires_high, effect -> pin_change);
      else                 list_insert(trigger -> wires_low , effect -> pin_change);
    else
      if (effect -> enter) list_insert(trigger -> enter_set , effect -> state_change);
      else                 list_insert(trigger -> leave_set , effect -> state_change);
...
Specification * extend_trigger(
  ...
  if (state_names) {
    ...
    for (iterate(state_names, char *, name)) {
      
      State * state = state_get(name);
      
      if (!state)
        yyerror("Unknown state name " CYAN "%s", name);
      
      list_insert(states, state);
    }
    
    state_names -> value_free = free;
    list_delete(state_names);
    state_names = NULL;
    
    trigger -> precondition = states;
  } else {
  ...
  for (iterate(all_sensors -> all, Sensor *, proto)) {
    ...
    for (int stream = 0; stream < proto -> data_streams; stream++) {
      ...
      for (iterate(proto -> outputs[stream].triggers, Trigger *, trigger)) {
        ...
        for (iterate(trigger -> wires_low, PinChange *, change))
          if (change -> delay) printf(YELLOW "-" RESET "%d(" YELLOW "%dms" RESET ") ", change -> gpio, change -> delay);
          else                 printf(YELLOW "-" RESET "%d ", change -> gpio);
        
        for (iterate(trigger -> wires_high, PinChange *, change))
          if (change -> delay) printf(YELLOW "+" RESET "%d(" YELLOW "%dms" RESET ") ", change -> gpio, change -> delay);
          else                 printf(YELLOW "+" RESET "%d ", change -> gpio);
        
        printf(GRAY "}\n        states { " RESET);
        
        for (iterate(trigger -> leave_set, StateChange *, change))
          if (change -> delay) printf(YELLOW "-" RESET "%s(" YELLOW "%dms" RESET ") ", change->state->name, change -> delay);
          else                 printf(YELLOW "-" RESET "%s ", change->state->name);
        
        for (iterate(trigger -> enter_set, StateChange *, change))
          if (change -> delay) printf(YELLOW "+" RESET "%s(" YELLOW "%dms" RESET ") ", change->state->name, change -> delay);
          else                 printf(YELLOW "+" RESET "%s ", change->state->name);
        
        printf(GRAY "}\n" RESET);
        
        if (trigger -> singular) printf(GRAY "    }\n");
        else                     printf(GRAY "    }" YELLOW " *\n" GRAY);
      }
    }
  }
}
